...
def brain(
    ...
    *,
    ...
    print(f"tools registered: {len(runtime.tool_registry.names())}")
    print("Three terminals to know:")
    print("  darwin chat               clean conversational REPL")
    print("  darwin inspect <command>  one-shot instrument query")
    print("  this terminal             live brain event stream")
    ...
    print(
        "\n".join(
            [
                ...
                "Type a path / shell command / URL / Python block and the",
                "intent router will invoke the matching tool automatically.",
                "",
                "  -- v5 instruments --",
                "/status         show Darwin's self-model",
                "/beliefs [cat]  causal beliefs (default: world-relevant; 'all' = include scheduler noise)",
                ...
                "/concept <name> a concept's definition, salience, neighbors",
                "/explain <s> <t>  every available proof chain between two concepts",
                "/infer          inferences fired on the last chat turn",
                "/reason         the last reasoning trace",
                ...
                "/categorize <summary|concept N> derived belief-category snapshot",
                "",
                "  -- evolution safeguards --",
                "/evolution                ledger summary + recent versioned mutations",
                ...
                "/tool <action> k=v ...    run a tool action now",
                "/autonomous               recent autonomous-runner task history",
                "",
                "  -- mysterio substrate --",
                "/snapshot /diff <a> <b> /quarantine /gate /proposals /meta-proposer",
                "/divergence /interior-trace /narrative /observer /research /worlds",
                "/modalities /generated /bus /embeddings /strategic /memory /operator-style",
                "",
