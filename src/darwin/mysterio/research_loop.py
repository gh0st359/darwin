...
``LiveResearcher`` reads the interior-track substrate, the divergence
reports, and the long-horizon thread state; it looks for stable internal
regularities worth probing — a recurring interior belief, a chronically
starved loop, a factor with persistent high prediction failure. For each
surviving finding it emits a `ResearchFinding` (persisted) and optionally
registers a new generative strategy on the meta-proposer.
...
enforces the only structural restriction in v9 — and it is for
*instrument integrity*, not secrecy: a registered strategy's
``target_paths`` may not overlap the DivergenceProbe, the SnapshotStore,
or the server event stream. Those are the lenses the operator watches
emergence through; if Darwin rewrites them, the operator loses visibility
...
class LiveResearcher:
    ...
    def investigate(self, runtime: Any) -> list[ResearchFinding]:
        ...
        new_findings.extend(self._investigate_interior_beliefs(runtime))
    ...
    def _investigate_interior_beliefs(self, runtime: Any) -> list[ResearchFinding]:
