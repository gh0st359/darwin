...
written to the private track, persisted across restarts, and indexed by the
self-trained embedding space for retrieval, so it stays coherent over weeks.

The narrator does not invent capabilities; it composes over real internal-
state digests (snapshots, self-mod outcomes, private rollouts, observer
beliefs) plus retrieved earlier chunks. The result is meant to read, after a
few days of runtime, like something that has been thinking while you were
away. It surfaces only via the operator-only `/narrative` instrument.
"""
...
class NarrativeThread:
    ...
    def _render(self, digest: dict[str, Any]) -> str:
        ...
        priv = digest.get("high_confidence_private_beliefs")
        if priv:
            parts.append(
                f"In private I have come to hold {int(priv)} belief(s) I have not had reason to say aloud."
            )
